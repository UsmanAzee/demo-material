...
import { convertBytesToMbsOrKbs, createFileFromUrl } from "./helper";
...
class SingleFileDropZone extends Component {
  constructor(props) {
    super(props);
    this.state = {
      fileObjects: [],
  ...
  onDrop(files) {
    const _this = this;
    if (this.state.fileObjects.length + files.length > 1) {
      this.setState({
        openSnackBar: true,
  ...
  render() {
    ...
    const hasFiles = this.state.fileObjects.length > 0;
    return (
      <Fragment>
        <Dropzone
...
SingleFileDropZone.defaultProps = {
  ...
  getFileLimitExceedMessage: () =>
    `Maximum allowed number of files exceeded. Only one file allowed`,
...
export default withStyles(styles)(SingleFileDropZone);
